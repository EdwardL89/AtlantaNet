...
from misc.tools import np_coor2xy, np_coory2v
from misc.tools import pano_connect_points
...
def create_occlusion_mask(xyz,W,H):
    xs, ys, zs = xyz.T
...
def show_3D_layout(img_name, layout_json,camera_height):
    #
    ppm = 100
    point_size = 0.0025
    ...
    equirect_texture = np.array(img_pil) / 255.0
    H, W = equirect_texture.shape[:2]
    ...
    # Warp each wall
    all_rgb, all_xyz = warp_walls(equirect_texture, floor_xy, floor_z, ceil_z, ppm)
    ...
    occlusion_mask, reord_idx = create_occlusion_mask(all_xyz,W,H)
